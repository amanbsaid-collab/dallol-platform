...
export async function dispatchScheduledWorkflows(now = new Date()) {
  ...
  const due = workflows.filter((workflow: (typeof workflows)[number]) => {
  ...
  for (const workflow of due) results.push(await emitSynticEvent({ organizationId: workflow.organizationId, type: 'schedule.tick', payload: { workflowId: workflow.id, scheduledAt: now.toISOString() } }));
